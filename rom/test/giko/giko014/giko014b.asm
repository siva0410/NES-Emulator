...
mainLoop:					; 메인루프
	...
	lda #$00
	sta $4016
	sta KeyBuffer	; 키 버퍼 초기화
	ldx #$08
	jmp getPadBitSub

getPadBit:
	asl <KeyBuffer	; 키 버퍼 좌 쉬프트
getPadBitSub:
	; 패드 입력 체크 루프
	lda $4016  		; 버튼
	and #1     		; AND #1
	ora <KeyBuffer	; 키 버퍼에 ora
	sta <KeyBuffer
	dex
	bne getPadBit	; 8회 반복한다(A,B,Select,Start,상,하,좌,우)

	lda <KeyBuffer
	lsr <KeyBuffer	; 키 버퍼 우 쉬프트
	and #1     		; AND #1
	beq lsr_A1		; 0이라면 lsr_A1으로 점프
	jsr RIGHTKEYdown
lsr_A1:
	lda <KeyBuffer
	lsr <KeyBuffer	; 키 버퍼 우 쉬프트
	and #1     		; AND #1
	beq lsr_A2		; 0이라면 lsr_A2으로 점프
	jsr LEFTKEYdown
lsr_A2:
	lda <KeyBuffer
	lsr <KeyBuffer	; 키 버퍼 우 쉬프트
	and #1     		; AND #1
	beq lsr_A3		; 0이라면 lsr_A3으로 점프
	jsr DOWNKEYdown
lsr_A3:
	lda <KeyBuffer
	lsr <KeyBuffer	; 키 버퍼 우 쉬프트
	lsr <KeyBuffer	; 키 버퍼 우 쉬프트(Start를 스킵)
	lsr <KeyBuffer	; 키 버퍼 우 쉬프트(Select를 스킵)
	and #1     		; AND #1
	beq lsr_A4		; 0이라면 lsr_A4으로 점프
	jsr UPKEYdown
lsr_A4:
	lda <KeyBuffer
	lsr <KeyBuffer	; 키 버퍼 우 쉬프트
	and #1     		; AND #1
	beq lsr_A5		; 0이라면 lsr_A5으로 점프
	jsr BKEYSound
lsr_A5:
	lda <KeyBuffer
