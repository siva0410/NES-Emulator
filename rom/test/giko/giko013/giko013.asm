...
mainLoop:					; 메인루프
	...
	lda $4016  ; A버튼
	and #1     ; AND #1
	beq isBKEYdown  ; 0이라면 IsBKEYdown으로 점프
	jsr AKEYSound

isBKEYdown:
	lda $4016  ; B버튼ン
	and #1     ; AND #1
	beq isSelectdown  ; 0이라면 isSelectdown으로 점프
	jsr BKEYSound
...
NOTHINGdown:
	...
	; 사운드 대기 카운터 A~D(제로 페이지로 연속된 영역이라는 전제)를 각각 -1 감산한다
	ldx #0
dec_Counter
	lda <Sound_A,x
	beq dec_Next
...
AKEYSound:
	; 사운드 대기 카운터 A가 0이 아닌 경우에는 사운드를 울리지 않는다
	lda <Sound_A
	beq AKEYSoundSub
	rts
AKEYSoundSub:
